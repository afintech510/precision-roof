  ...
  "findings": [
    {
      ...
      "category": "[specific subcategory — e.g., 'Missing Index', 'Referential Integrity', 'Data Type']",
...
- Be adversarial. Question every assumption. If the spec says a page "reads from town pricing," ask: what if the record is missing? Stale? What's the type/precision of the money field?
...
Additional FOUNDATION emphasis: For every API endpoint (§3), trace it to its underlying data (the Sanity content model §2.1–2.4 and the operational store §2.5). Can the schema actually produce the promised response shape? For every SOW feature, verify data support exists — e.g., F-014's quote widget must have `townPricing` records for every town×service×home-size band it will be asked about, or it degrades to an error path. Check that money uses integer cents or fixed precision (not floats), timestamps are timezone-aware (`timestamptz`), enums are constrained, the `opt_out`/`message_log`/`review_request` tables have the indexes their lookups need, and the required-field publish gate (F-003) actually covers every field the SOW §4 depth standard lists. Confirm the split between Sanity (content) and the operational store (leads/messages) has no referential gap where a webhook needs to join across the two systems.
